...
export function ChatDashboard({ role, roomName }: ChatDashboardProps) {
  const [text, setText] = useState("");
  const messagesEndRef = useRef<HTMLDivElement>(null);
  const [soundEnabled, setSoundEnabled] = useState(false);

  const { messages, onlineCount, connectedUsers, alert, sharedActions, sendMessage, sendAlert, updateSharedSettings } = useSocket(roomName, role, soundEnabled);
  ...
  const MobileQuickActions = () => (
    <div className="grid grid-cols-2 lg:grid-cols-1 gap-2 p-4 lg:p-0">
      <h3 className="lg:block hidden text-neutral-400 font-medium mb-2">Acciones Rápidas</h3>
      ...
      <header className="h-16 lg:h-20 bg-neutral-900 border-b border-neutral-800 flex items-center justify-between px-4 lg:px-6 shrink-0">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl lg:text-3xl font-bold tracking-tight">
            StreamSync <span className="text-neutral-500 font-normal">| {role}</span>
          </h1>
        </div>

        <div className="flex items-center gap-2 lg:gap-4">
          <QuickActionsSettings actions={quickActions} onSave={handleSaveActions} />
          <Button
            variant="ghost"
            size="icon"
            ...
            <SheetContent side="left" className="w-[300px] bg-neutral-900 border-neutral-800 text-white p-0">
              <div className="p-4 border-b border-neutral-800 flex items-center justify-between">
                <h2 className="font-bold text-lg flex items-center gap-2">
                  <Users className="w-5 h-5 text-green-500" /> Conectados ({onlineCount})
                </h2>
              </div>
                ...
                {connectedUsers.map((user) => (
                  <div key={user.id} className="flex items-center gap-3 p-3 rounded-xl bg-neutral-950 border border-neutral-800">
                    ...
                    <span className="font-medium text-sm">
                      {user.role} {user.role === role ? "(Tú)" : ""}
                    </span>
                    <span className="ml-auto text-[10px] text-green-500 border border-green-500/30 bg-green-500/10 px-2 py-0.5 rounded-full font-bold uppercase tracking-wider">Online</span>
                  </div>
                ))}
              </div>
            </SheetContent>
        ...
        <div className="hidden lg:flex w-64 bg-neutral-900 border-r border-neutral-800 flex-col py-6 px-4">
          ...
          <div className="space-y-2 flex-1 overflow-y-auto">
            {connectedUsers.map((user) => (
             <div key={user.id} className="flex items-center gap-3 p-3 rounded-xl bg-neutral-950 border border-neutral-800">
               <div className={`w-3 h-3 rounded-full ${ROLE_COLORS[user.role as Role] || 'bg-neutral-500'} shadow-[0_0_8px_currentColor]`} />
               <span className="font-medium text-sm">{user.role} {user.role === role ? "(Tú)" : ""}</span>
               <span className="ml-auto text-[10px] text-green-500 border border-green-500/30 bg-green-500/10 px-2 py-0.5 rounded-full font-bold uppercase tracking-wider">Online</span>
             </div>
            ))}
          </div>
          <div className="text-xs text-neutral-500 bg-neutral-950 p-4 rounded-xl mt-4 shrink-0">
            <span className="block mb-2 font-bold text-neutral-400">Teclado Shortcuts</span>
            ...
            {quickActions.filter(a => a.shortcut).length === 0 && (
              <span className="italic block mt-1">Sin atajos (Configura en ⚙️)</span>
            )}
          </div>
        </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 lg:p-8 space-y-4 lg:space-y-6">
            {messages.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-neutral-600">
                 ...
                 <p className="text-xl lg:text-3xl font-medium">Transmisión Iniciada</p>
                 <p className="text-sm lg:text-lg opacity-60">Esperando indicaciones...</p>
              </div>
            ) : (
              messages.map((msg) => (
                <div
