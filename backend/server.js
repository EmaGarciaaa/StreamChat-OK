...
// Enable CORS for frontend hosting (like Vercel)
app.use(cors({ origin: "*" }));
...
const roomTokens = new Map(); // RoomID -> Set of valid session tokens
...
app.post("/api/room/:roomId/verify-pin", (req, res) => {
  const roomId = req.params.roomId;
  const { pin } = req.body;
...
io.on("connection", (socket) => {
  ...
  socket.on("update-settings", (data) => {
    if (!socket.data || !socket.data.authenticated || socket.data.room !== data.room) return;
...
app.get("/health", (req, res) => {
  res.json({ status: "ok", service: "streamsync-backend" });
});

httpServer.listen(PORT, "0.0.0.0", () => {
  console.log(`Backend de StreamSync corriendo en el puerto ${PORT}`);
});
